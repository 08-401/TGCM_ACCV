...
def train(epoch, model, dataloader, optimizer, training):
    ...
    for idx, batch in enumerate(dataloader):
        ...
        #         img2,mask2,
        #         beta_factor=0.4)
        #     batch['query_img'], batch['query_mask'], partMaskList= cutmix4query_segmentation(
        #         batch['query_img'],
        #         batch['query_mask'],
        #         img2,mask2,
        #         beta_factor=0.4)
        ...
        if training:
            logit_mask = model(batch['query_img'], batch['support_imgs'].squeeze(1), batch['support_masks'].squeeze(1),img2,mask2)
        else:
        ...
        #     for i in range(bsz):
        #         a = logit_mask[i,:, 128:128 + H, 128:128 + H].reshape(1,2,-1).cuda()
        #         b = partMaskList[i].reshape(1, -1).long().cuda()
        #         lossi += loss_f(a, b)
